Update `elixir/flake.nix`, `elixir-app/flake.nix`, and `gleam/flake.nix` so
their `beamVersion` and `elixirVersion` bindings track the latest versions
...
- If there is a diff, write a unified patch file to
  `generated-patches/beam-version-updates.patch`.
...
3. If `git diff -- elixir/flake.nix elixir-app/flake.nix gleam/flake.nix` is
   non-empty, save it as `generated-patches/beam-version-updates.patch`.
